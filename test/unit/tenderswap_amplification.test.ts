...
import { ITenderSwap, SimpleToken, LiquidityPoolToken, IERC20 } from '../../typechain'
  ...
  describe('Check for timestamp manipulations', () => {
    ...
    describe('Check for attacks while A is ramping upwards', () => {
      ...
      beforeEach(async () => {
        ...
        initialPoolBalances = [
          await swap.getToken0Balance(),
          await swap.getToken1Balance()
        ]
      ...
      describe(
        ...
        () => {
          it('Attack fails with 900 seconds between blocks', async () => {
            ...
            await swap
              .connect(attacker)
              .swap(firstToken.address, String(1e18), 0, constants.MaxUint256)
            ...
            expect(await swap.getToken0Balance()).to.be.eq(String(2e18))
            expect(await swap.getToken1Balance()).to.be.eq('91408257454997694')
            ...
            await swap
              .connect(attacker)
              .swap(secondToken.address, secondTokenOutput, 0, constants.MaxUint256)
            ...
            finalPoolBalances.push(await swap.getToken0Balance())
            finalPoolBalances.push(await swap.getToken1Balance())
      ...
      describe(
        ...
        () => {
          ...
          it('Attack fails with 900 seconds between blocks', async () => {
            ...
            expect(await swap.getToken0Balance()).to.be.eq(String(2e18))
            expect(await swap.getToken1Balance()).to.be.eq(
              '1988066748939318647'
            )
