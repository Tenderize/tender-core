...
describe('TenderToken', () => {
  ...
  it('permit', async () => {
    ...
    const nonce = await tenderToken.nonces(account0)
    const deadline = ethers.constants.MaxUint256
    const digest = await getApprovalDigest(
      tenderToken,
      { owner: account0, spender: account1, value: TEST_AMOUNT },
      nonce,
      deadline
    )
    ...
    expect(await tenderToken.nonces(account0)).to.eq(BigNumber.from(1))
  })
})
