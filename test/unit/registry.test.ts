...
describe('Tenderizer Registry', () => {
  ...
  describe('add tenderizer', () => {
    const config = {
      ...
      tenderSwap: ethers.utils.getAddress('0x3cD751E6b0078Be393132286c442345e5DC49699'),
    ...
    it('emits a TenderizerCreated event', async () => {
      const arr = [config.name, config.controller, config.steak, config.tenderizer, config.tenderToken, config.tenderSwap, config.tenderFarm]
