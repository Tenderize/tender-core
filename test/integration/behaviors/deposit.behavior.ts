...
export default function suite () {
  ...
  describe('deposits funds succesfully', async () => {
    ...
    it('increases TenderToken supply', async () => {
      expect(await ctx.TenderToken.totalSupply()).to.eq(ctx.deposit.add(ctx.initialStake))
    })

    it('increases Tenderizer principle', async () => {
      expect(await ctx.Tenderizer.currentPrincipal()).to.eq(ctx.deposit.add(ctx.initialStake))
    })
